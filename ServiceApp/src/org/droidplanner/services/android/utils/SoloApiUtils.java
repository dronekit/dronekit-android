...
public class SoloApiUtils {
    ...
    public static void startVideoStream(DroneManager droneManager, int apiVersionCode, String appId, String videoTag, Surface videoSurface,
                                        ICommandListener listener) {
        if (!isSoloLinkFeatureAvailable(droneManager, listener))
            return;

        final SoloComp soloComp = droneManager.getSoloComp();
        soloComp.startVideoStream(apiVersionCode, appId, videoTag, videoSurface, listener);
    }

    public static void stopVideoStream(DroneManager droneManager, int apiVersionCode, String appId, String videoTag, ICommandListener listener) {
        if (!isSoloLinkFeatureAvailable(droneManager, listener))
            return;

        final SoloComp soloComp = droneManager.getSoloComp();
        soloComp.stopVideoStream(apiVersionCode, appId, videoTag, listener);
    }
}
